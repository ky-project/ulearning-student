...
export default {
  ...
  methods: {
    ...
    getDocumentRoot(data) {
      return new Promise((resolve, reject) => {
        this.loading = true
        axiosGet(GET_DOCUMENT_ROOT_URL, { params: data })
          .then(response => {
            this.loading = false
            resolve(response)
          })
    ...
    getDocumentList(data) {
      return new Promise((resolve, reject) => {
        ...
        this.loading = true
        axiosGet(GET_DOCUMENT_LIST_URL, { params: data })
          .then(response => {
    ...
    downloadFile(fileName, id) {
      ...
      const fileurl = process.env.VUE_APP_BASE_API + DOWNLOAD_DOCUMENT_URL + '?id=' + id
      a.href = fileurl
